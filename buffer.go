...
// Simple message buffer factory.
type SimpleBufferFactory struct {
}

// Create a message buffer.
func (_ SimpleBufferFactory) NewBuffer() Buffer {
...
// Get internal buffer.
func (m *SimpleBuffer) Data() []byte {
	return m.b
}

// Get buffer length.
func (m *SimpleBuffer) Len() int {
	return len(m.b)
}

// Get buffer capacity.
func (m *SimpleBuffer) Cap() int {
	return cap(m.b)
}

// Prepare buffer for next read.
func (m *SimpleBuffer) PrepareRead(size int) {
	if cap(m.b) >= size {
		m.b = m.b[0:size]
	} else {
		m.b = make([]byte, size)
...
func (m *SimpleBuffer) PrepareWrite(size int) {
	if cap(m.b) >= size {
		m.b = m.b[0:0]
	} else {
...
func (m *SimpleBuffer) Append(p ...byte) {
	m.b = append(m.b, p...)
}

func (m *SimpleBuffer) Ignore(n int) {
...
func (m *SimpleBuffer) Write(p []byte) (int, error) {
	m.b = append(m.b, p...)
	return len(p), nil
}
...
func (m *BufferReader) Read(p []byte) (int, error) {
	if m.i == m.b.Len() {
		return 0, io.EOF
	}
	n := copy(p, m.b.Data()[m.i:])
	m.i += n
	return n, nil
}
...
func (m *BufferReader) ReadRune() rune {
	r, size := utf8.DecodeRune(m.b.Data()[m.i:])
	m.i += size
	return r
}
...
func (m *BufferReader) ReadUint16LE() uint16 {
	r := binary.LittleEndian.Uint16(m.b.Data()[m.i:])
	m.i += 2
	return r
}
...
func (m *BufferReader) ReadUint16BE() uint16 {
	r := binary.BigEndian.Uint16(m.b.Data()[m.i:])
	m.i += 2
	return r
}
...
func (m *BufferReader) ReadUint32LE() uint32 {
	r := binary.LittleEndian.Uint32(m.b.Data()[m.i:])
	m.i += 4
	return r
}
...
func (m *BufferReader) ReadUint32BE() uint32 {
	r := binary.BigEndian.Uint32(m.b.Data()[m.i:])
	m.i += 4
	return r
}
...
func (m *BufferReader) ReadUint64LE() uint64 {
	r := binary.LittleEndian.Uint64(m.b.Data()[m.i:])
	m.i += 8
	return r
}
...
func (m *BufferReader) ReadUint64BE() uint64 {
	r := binary.BigEndian.Uint64(m.b.Data()[m.i:])
	m.i += 8
	return r
}
