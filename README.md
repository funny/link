...
所以经过不断的提炼，就像在简化公式一样的，link最后变得十分的简单，同时它的定位也很清楚。link不是一个网络层也不是一个框架，它只是一个脚手架，它可以帮助你快速的实现出你所需要的网络层或者框架，帮你约束网络层的实现方式，但是它不会管更多的事情。
...
这里面实现了常见的Json、Gob、Xml格式的消息封包解包，正好这三种消息类型都不涉及到分包协议，所以可以很容易的内置到link包里。

早版本的link包曾经尝试加入分包协议的支持，但是分包协议变化众多，有点像前面提到的`AsyncSend()`一样众口难调，所以就去掉了。

将来会考虑像Erlang一样内置几个简单易用的分包形式，作为参考也满足一些要求不高的场景使用。

[codec_safe.go]()
-----------------

这里面实现了线程安全的`CodecType`，旧版本的link里，`Session`内置了收发锁，让`Session.Receive()和`Session.Send()`可以被并发调用。但是实际项目中并接受或者并发发送的场景很少，如果一开始就内置到`Session`里，这部分调用开销就是多余的了。
...
从link的核心代码和内置类型可以看出，核心其实很简单CodecType的设计很灵活，所以在实际项目中可以根据自己的需求参考内置的类型实现针对项目的CodecType，这样可以获得最好的执行效率和使用体验。
